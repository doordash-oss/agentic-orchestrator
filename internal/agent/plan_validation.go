	...
	for attempt := loopStart; attempt <= maxAttempts; attempt++ {
		...
		if resumeValidation {
			...
			cmd, env, sessOpts, err := cfg.BuildSession(BuildSessionOpts{
				...
				PIDDir:                         pidDir,
				PermHandler:                    permHandlerFor(cfg.DangerouslySkipPermissions, cfg.AutoReview, cfg.PermissionCache, cfg.RepoName, cfg.Classify, autoReviewDecisionHook(cfg.Observer, cfg.Feature)),
				WorkDir:                        cfg.WorkDir,
				EffortLevel:                    cfg.EffortLevel,
				Phase:                          feature.PhasePlan,
