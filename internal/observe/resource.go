...
func forbiddenResourceAttribute(key, value string) bool {
	for _, token := range []string{"user", "employee", "team", "host", "machine", "repository", "repo", "workspace", "config", "path", "directory", "token", "secret", "password", "credential", "authorization"} {
		if strings.Contains(key, token) {
			return true
		}
	}
	return absolutePathPattern.MatchString(value) || permission.RedactTelemetryText(value) != value
}
