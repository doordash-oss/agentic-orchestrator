...
A design smell is Critical or High only when it materially obscures purpose, hierarchy, relationships, readability, interaction, accessibility, responsive use, or a committed visual direction. Systemic issues that reduce coherence or polish while the interface stays understandable are Medium; localized refinement is Low. Never block because you would prefer a different font, palette, density, or stylistic trend, and accept a deliberate rule-breaking choice when its purpose is evident and the surrounding system supports it.
