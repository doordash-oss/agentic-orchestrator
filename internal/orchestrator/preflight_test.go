...
func TestRefactorPreflightResolvesAllScope(t *testing.T) {
	t.Parallel()
	o := newPreflightOrchestrator(t, nil)
	result, err := o.RefactorPreflight("feat-preflight", "", "rename foo", "")
	if err != nil {
		t.Fatalf("RefactorPreflight: %v", err)
	}
	if result.Scope != preflightScopeAll {
		t.Fatalf("scope = %q; want all", result.Scope)
	}
	if len(result.Repos) != 2 {
		t.Fatalf("repos = %v; want both feature repos", result.Repos)
	}
	if result.SourceRevision == "" {
...
func TestRefactorPreflightResolvesSingleScope(t *testing.T) {
	t.Parallel()
	o := newPreflightOrchestrator(t, nil)
	result, err := o.RefactorPreflight("feat-preflight", preflightRepoA, "rename foo", "")
	if err != nil {
		t.Fatalf("RefactorPreflight: %v", err)
	}
	if result.Scope != preflightScopeSingle {
		t.Fatalf("scope = %q; want single", result.Scope)
	}
	if len(result.Repos) != 1 || result.Repos[0] != preflightRepoA {
		t.Fatalf("repos = %v; want [repo-a]", result.Repos)
	}
}

func TestRefactorPreflightReportsBlockerForUnknownRepo(t *testing.T) {
	t.Parallel()
	o := newPreflightOrchestrator(t, nil)
	result, err := o.RefactorPreflight("feat-preflight", "no-such-repo", "rename foo", "")
	if err != nil {
		t.Fatalf("RefactorPreflight: %v", err)
	}
	if len(result.Blockers) == 0 {
		t.Fatal("expected a blocker for an unknown repo")
	}
}
