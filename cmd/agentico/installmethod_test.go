...
func TestClassifyInstallMethod(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "homebrew precedence: the Cellar signal wins over a real module version in build info",
			buildInfoVersion: "v1.2.3",
			injectedVersion:  "1.2.3",
		...
		{
			name:             "homebrew precedence: the Cellar signal wins when the brew prefix coincides with the go bin dir",
			buildInfoVersion: "(devel)",
			injectedVersion:  "1.2.3",
			binaryDir:        goBin,
			goBinDir:         goBin,
		...
		{
			name:             "go-install still classifies when there is no homebrew signal",
			buildInfoVersion: "v1.2.3",
			injectedVersion:  "1.2.3",
			binaryDir:        goBin,
			goBinDir:         goBin,
