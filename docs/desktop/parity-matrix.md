...
| Keyboard-first operation (every action reachable without pointer) | All keybindings (`internal/tui/keys.go`) | Full keyboard map + focus management, platform accelerators | local | macOS+Linux | partially: focus/keyboard tests in Task 1 shell (`ConnectionShell.test.tsx`); Task 4 wizard keyboard-only traversal, error focus management, and consent-dialog Escape handling (`SetupWizard.test.tsx`) | pending |
