...
- Prefer evidence producible headlessly from the repository. For Slack-style integrations, use JSON snapshot or golden tests plus captures of Agentico's own UI; a third-party authenticated UI capture is a last resort and must carry `[agentico capability: authenticated-browser(<host>)]`. A Visual Evidence or Manual Verification row that names an external host without a capability tag is returned for plan revision. Rows needing a login, device, or service declare one built-in capability (`authenticated-browser(<host>)`, `display`, `docker`, `network(<host[:port]>)`); syntax and probe timing are in format.md.
