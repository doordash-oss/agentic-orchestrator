 ...
 * Everything here fails closed with typed canonical errors.
 */
  ...
  while (stack.length > 0) {
    ...
    for (const key of Object.getOwnPropertyNames(current)) {
      ...
      if (FORBIDDEN_KEYS.has(key)) {
        throw new CanonicalErrorException(buildCanonicalError('E_UNSAFE_PAYLOAD'));
      }
...
export function assertWithinByteSize(text: string, maxBytes: number = MAX_PAYLOAD_BYTES): void {
  ...
  if (bytes > maxBytes) {
    throw new CanonicalErrorException(
      buildCanonicalError('E_PAYLOAD_TOO_LARGE', {
        params: { bytes: String(bytes), limit: String(maxBytes) },
      }),
    );
  }
}
