...
func TestPipelineProfilePhaseProgression(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantPhases: []Phase{PhaseKnowledgeBase, PhaseInquire, PhaseResearch, PhaseBrainstorm, PhasePlan, PhaseImplement, PhaseReview, PhasePublish},
			hasPhase:   PhaseResearch,
			wantHas:    true,
		},
		{
...
func TestMinimumProfileForPhase(t *testing.T) {
	...
	tests := []struct {
		...
		{"Brainstorm requires large", PhaseBrainstorm, PipelineLarge},
