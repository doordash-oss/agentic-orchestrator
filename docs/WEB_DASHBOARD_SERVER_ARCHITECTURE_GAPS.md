...
| Create wizard draft persistence | The create-feature wizard keeps draft state only in React memory. Refreshes, Vite restarts, backend auth/discovery changes, or repo-loading failures can lose the entered name, description, and selections before submit. | Users can lose the feature brief while troubleshooting setup issues or switching repos, which is especially painful because the description is the main guidance/context for the agents. | Persist the draft locally (`name`, `description`, selected repos, pipeline/review options) and clear it only after successful create or explicit cancel/reset. |
