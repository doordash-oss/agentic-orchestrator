...
func TestUserFacingDocsDescribeAutomaticBashReviewContract(t *testing.T) {
	...
	requiredByDoc := map[string][]string{
		"README.md": {
			...
			"new sessions",
			"up to two low-effort attempts",
			"one overall one-minute timeout",
			"ordinary human prompt",
			"Auto-approved Bash: ",
			"Auto-approved Bash (fast path): ",
			"dangerous commands",
			"fallible, promptable component",
		...
		filepath.Join("skills", "chat", "user-guide", "configuration.md"): {
			...
			"fast path, not reviewer eligibility",
