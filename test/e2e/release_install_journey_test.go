...
func TestReleaseInstallJourneyNegative(t *testing.T) {
	selfupdateJourneyGuard(t)
	cases := []struct {
		name        string
		spec        releaseServeSpec
		wantFailure string
		// preStaging marks failures that must happen before any staging (and
		// therefore before any probe can run).
		preStaging bool
	}{
		{
			name:        "tampered manifest",
			spec:        releaseServeSpec{tag: "v" + releaseInstallTarget, mode: releaseServeTamperManifest, envelope: true},
			wantFailure: "selfupdate-driver: release verification failed",
			preStaging:  true,
		},
		{
			name:        "foreign signing key",
			spec:        releaseServeSpec{tag: "v" + releaseInstallTarget, mode: releaseServeForeignSigner},
			wantFailure: "selfupdate-driver: release verification failed",
			preStaging:  true,
		},
		{
			name:        "invalid advertised envelope",
			spec:        releaseServeSpec{tag: "v" + releaseInstallTarget, mode: releaseServeEnvelopeMismatch, envelope: true},
			wantFailure: "selfupdate-driver: release verification failed",
			preStaging:  true,
		},
		{
			name:        "unsafe archive",
			spec:        releaseServeSpec{tag: "v" + releaseInstallTarget, mode: releaseServeUnsafeArchive},
			wantFailure: "selfupdate-driver: release staging failed",
			preStaging:  false,
		},
		{
			name:        "failing probe",
			spec:        releaseServeSpec{tag: "v" + releaseInstallTarget, mode: releaseServeWrongBanner},
			wantFailure: "selfupdate-driver: release probe failed",
			preStaging:  false,
		},
		{
			name:        "non-increasing target",
			spec:        releaseServeSpec{tag: "v" + releaseInstallLower, mode: releaseServeNonIncreasing, envelope: false},
			wantFailure: "selfupdate-driver: release staging failed",
			preStaging:  false,
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			f := newReleaseServeFixture(t)
			spec := tc.spec
			if spec.mode == releaseServeWrongBanner {
				// The archive carries the lower build: the probe banner
				// cannot report the advertised target version.
				spec.executable = bins.lower
			}
			if spec.mode == releaseServeNonIncreasing {
				spec.executable = bins.lower
			}
			f.publishRelease(t, spec)
			...
			for time.Now().Before(deadline) {
				if p.stderrContains(tc.wantFailure) {
					break
				}
				if !p.alive() {
					t.Fatalf("driver died before reporting %q (stderr tail:\n%s)", tc.wantFailure, p.stderrTail())
				}
				time.Sleep(50 * time.Millisecond)
			}
			if !p.stderrContains(tc.wantFailure) {
				t.Fatalf("failure %q never reported (stderr tail:\n%s)", tc.wantFailure, p.stderrTail())
			}
			if tc.preStaging && p.stderrContains("selfupdate-driver: release staged ") {
				t.Fatalf("failure before authentication still staged a release (stderr tail:\n%s)", p.stderrTail())
			}
			// The old build remains installed and healthy: no shutdown began.
			post := j.waitHealthy(p, baseURL, releaseInstallLower, 10*time.Second)
			if post.Owner.PID != p.pid() {
				t.Fatalf("owner pid = %d; want preserved %d", post.Owner.PID, p.pid())
			}
			...
			if _, err := os.Stat(j.receiptPath()); !os.IsNotExist(err) {
				t.Fatalf("negative journey wrote a receipt: %v", err)
			}
			if n := p.stderrPrefixCount("selfupdate-driver: exec /"); n != 0 {
				t.Fatalf("invalid target reached exec: %d crossings", n)
			}
			if code := p.terminate(); code != 0 {
				t.Fatalf("SIGTERM exit code = %d; want 0 (stderr tail:\n%s)", code, p.stderrTail())
			}
...
func TestReleaseInstallJourneySubstitution(t *testing.T) {
	selfupdateJourneyGuard(t)
	j, bins := newReleaseInstallJourney(t)
	f := newReleaseServeFixture(t)
	...
	port := selfupdateFreePort(t, "127.0.0.1")
	baseURL := "http://" + net.JoinHostPort("127.0.0.1", strconv.Itoa(port))
	p := j.startRelease(f.server.URL, "127.0.0.1:"+strconv.Itoa(port), "--fail-at", "substitute-candidate")
	j.waitHealthy(p, baseURL, releaseInstallLower, 30*time.Second)
	j.trigger()

	deadline := time.Now().Add(30 * time.Second)
	for time.Now().Before(deadline) && !p.stderrContains("selfupdate-driver: release begin failed") {
		time.Sleep(50 * time.Millisecond)
	}
	if !p.stderrContains("selfupdate-driver: release begin failed") {
		t.Fatalf("substitution never refused (stderr tail:\n%s)", p.stderrTail())
	}
	// The old build remains installed and healthy; no receipt exists.
	post := j.waitHealthy(p, baseURL, releaseInstallLower, 10*time.Second)
	if post.Owner.PID != p.pid() {
		t.Fatalf("owner pid = %d; want preserved %d", post.Owner.PID, p.pid())
	}
	...
	if _, err := os.Stat(j.receiptPath()); !os.IsNotExist(err) {
		t.Fatalf("substitution journey wrote a receipt: %v", err)
	}
	if code := p.terminate(); code != 0 {
		t.Fatalf("SIGTERM exit code = %d; want 0 (stderr tail:\n%s)", code, p.stderrTail())
	}
	// The abandoned release staging is recognized owned staging: an ordinary
	// relaunch cleans it without installing anything.
	relaunchPort := selfupdateFreePort(t, "127.0.0.1")
	relaunchBase := "http://" + net.JoinHostPort("127.0.0.1", strconv.Itoa(relaunchPort))
	relaunch := j.launch("selfupdate-driver", "--config", j.configPath, "--state-dir", j.stateDir, "--listen", "127.0.0.1:"+strconv.Itoa(relaunchPort))
	j.waitHealthy(relaunch, relaunchBase, releaseInstallLower, 30*time.Second)
	releaseWaitNoStaging(t, j, 10*time.Second)
	if _, err := os.Stat(j.receiptPath()); !os.IsNotExist(err) {
		t.Fatalf("relaunch wrote a receipt: %v", err)
	}
	if got := j.installDigest(); got != bins.lowerDigest {
		t.Fatalf("installed digest = %s after relaunch; want unchanged %s", got, bins.lowerDigest)
	}
	if code := relaunch.terminate(); code != 0 {
		t.Fatalf("relaunch SIGTERM exit code = %d; want 0 (stderr tail:\n%s)", code, relaunch.stderrTail())
	}
}
...
func TestReleaseInstallJourneyStagingInterruption(t *testing.T) {
	selfupdateJourneyGuard(t)
	j, bins := newReleaseInstallJourney(t)
	f := newReleaseServeFixture(t)
	f.publishRelease(t, releaseServeSpec{tag: "v" + releaseInstallTarget})
	...
	port := selfupdateFreePort(t, "127.0.0.1")
	baseURL := "http://" + net.JoinHostPort("127.0.0.1", strconv.Itoa(port))
	barrierFile := filepath.Join(t.TempDir(), "barrier")
	p := j.startRelease(f.server.URL, "127.0.0.1:"+strconv.Itoa(port), "--barrier", "release-staged", "--barrier-file", barrierFile)
	j.waitHealthy(p, baseURL, releaseInstallLower, 30*time.Second)
	j.trigger()

	// Interrupt after release staging but before any install receipt.
	...
	for time.Now().Before(deadline) {
		...
		if !p.alive() {
			t.Fatalf("driver died before the barrier (stderr tail:\n%s)", p.stderrTail())
		}
		time.Sleep(25 * time.Millisecond)
	}
	if _, err := os.Stat(barrierFile); err != nil {
		t.Fatalf("barrier never reached (stderr tail:\n%s)", p.stderrTail())
	}
	if err := p.cmd.Process.Kill(); err != nil {
		t.Fatalf("kill at barrier: %v", err)
	}
	p.waitBounded(10 * time.Second)

	if _, err := os.Stat(j.receiptPath()); !os.IsNotExist(err) {
		t.Fatalf("interrupted staging left an install receipt: %v", err)
	}
	if got := j.installDigest(); got != bins.lowerDigest {
		t.Fatalf("installed digest = %s; want unchanged %s", got, bins.lowerDigest)
	}
	if dirs := releaseLeaseTxDirs(t, j); len(dirs) == 0 {
		t.Fatal("expected the interrupted release staging dir to survive the kill")
	}

	// An ordinary relaunch cleans the recognized abandoned staging safely,
	// retains no consent, and installs nothing.
	relaunch := j.launch("selfupdate-driver", "--config", j.configPath, "--state-dir", j.stateDir)
	_ = relaunch
	releaseWaitNoStaging(t, j, 15*time.Second)
	if _, err := os.Stat(j.receiptPath()); !os.IsNotExist(err) {
		t.Fatalf("relaunch wrote an install receipt: %v", err)
	}
	if got := j.installDigest(); got != bins.lowerDigest {
		t.Fatalf("installed digest = %s after relaunch; want unchanged %s", got, bins.lowerDigest)
	}
	requireNoSuppression(t, j, releaseInstallTarget)
}
