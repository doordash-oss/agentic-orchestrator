...
func TestExecuteRecovery_MultiRepoTweak_Kill_FailsRepoCycle(t *testing.T) {
	dir := t.TempDir()
	store := feature.NewStore(dir)
	cfg := config.NewDefault()
	fm := feature.NewManager(store, cfg)
	...
	f := &feature.Feature{
		ID:            "feat-mr-tweak-kill",
		Name:          "Multi Repo Tweak Kill",
		Slug:          "mr-tweak-kill",
	...
	// Seed RepoCycles with a running tweak for "api"
	_ = store.Modify(f.ID, func(f *feature.Feature) error {
		f.RepoCycles = map[string]*feature.RepoCycleState{
			"api": {Type: feature.CycleTweak, Status: "running"},
		}
		return nil
	})
	// Reload to get the updated feature with RepoCycles
	f, _ = fm.Get(f.ID)

	pidDir := filepath.Join(dir, f.ID)
	_ = WritePIDFile(pidDir, PIDFile{
		PID:       999999999,
		FeatureID: f.ID,
		Phase:     "implement",
		Dir:       pidDir,
	...
	actions := map[string]RecoveryAction{
		RecoveryActionKey(f.ID, "api"): RecoveryKill,
	}
	...
	// Feature should NOT be Interrupted — only the repo cycle should be failed
	if updated.Status == feature.StatusInterrupted {
		t.Errorf("Status = Interrupted, want anything but Interrupted (repo cycle should be failed, not feature)")
	}

	// RepoCycles["api"].Status must be "failed"
	if updated.RepoCycles == nil {
		t.Fatal("RepoCycles is nil, expected non-nil with failed api entry")
	}
	rc, ok := updated.RepoCycles["api"]
	if !ok {
		t.Fatal("RepoCycles[\"api\"] not found")
	}
	if rc.LastError == "" {
		t.Errorf("RepoCycles[\"api\"].Status = %q, want %q", rc.Status, "failed")
	}
