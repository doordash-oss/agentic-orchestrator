...
| --- | --- | --- | --- | --- | --- | --- |
| Connection shell: branded startup surface showing the connect lifecycle (resolve runtime → discover → attach → authenticate → ready) with status detail, error + retry | Implicit: terminal launch banner / welcome screen (`internal/tui/welcome.go`) | Instrument card with PhaseSpine rail, live status region, error panel with Retry | IPC `agentico:connection:get-status`, `agentico:connection:changed`; gateway stub until Task 3 (then `GET /api/v1/health`) | macOS+Linux | `desktop/src/renderer/src/components/ConnectionShell.test.tsx`, `desktop/src/renderer/src/components/PhaseSpine.test.tsx`, `desktop/src/main/__tests__/connection.test.ts` | delivered (stub gateway; real transport is Task 3) |
