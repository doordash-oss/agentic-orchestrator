...
func TestHTTPMetricsBoundsUnknownMethodsAndRunTails(t *testing.T) {
	for _, target := range []string{
		"/api/v1/features/f/runs/1/sessions/private-value",
		"/api/v1/features/f/runs/1/logs/private-value/more",
		"/api/v1/features/f/runs/1/artifacts/private-value/more",
	} {
		route, ok := telemetryRoute(httptest.NewRequest("PRIVATE-METHOD", target, nil))
		if !ok || strings.Contains(route, "private-value") || strings.Contains(route, "more") {
			t.Fatalf("unbounded route=%s", route)
		}
	}
	if got := telemetryMethod("PRIVATE-METHOD"); got != "_OTHER" {
		t.Fatalf("unbounded method=%s", got)
	}
	for _, path := range []string{apiPathUpdate, apiPathUpdateInstall, apiPathRuntimeReadiness, apiPathWorkspaceRepositoriesCreate} {
		route, ok := telemetryRoute(httptest.NewRequest(http.MethodGet, path, nil))
		if !ok || route != path {
			t.Fatalf("current route=%s want=%s", route, path)
		}
	}
}
