...
func TestMetricModelsIncludeCurrentCodexCatalogAndRejectUnknownIDs(t *testing.T) {
	for _, model := range []string{"gpt-6-astra", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.5", "gpt-5.4-mini"} {
		if got := metricEnum("model", "openai/"+model+"[1M]"); got != model {
			t.Errorf("model %s normalized to %s", model, got)
		}
	}
	for _, model := range []string{"gpt-9.99", "private-custom-model", "gpt-6-astra-secret"} {
		if got := metricEnum("model", model); got != "other" {
			t.Errorf("unknown model %s normalized to %s", model, got)
		}
	}
}

func TestFeatureMilestonesSurviveRestartAndMeasureEachRun(t *testing.T) {
	dir := t.TempDir()
	store := feature.NewStore(dir)
	start := time.Now().Add(-time.Minute)
	f := &feature.Feature{ID: "f1", Name: "test", Created: start.Add(-24 * time.Hour), Status: feature.StatusImplementing, ActiveRun: 1, RunCount: 1, SchemaVersion: feature.SchemaVersionCurrent}
	f.SetRun(&feature.Run{RunNumber: 1, StartedAt: &start})
	store.SetMutationObserver(&Observer{enabled: true, localEnabled: true, emitter: NewEmitter(dir)})
	if err := store.Save(f); err != nil {
		t.Fatal(err)
	}
	setStatus := func(status feature.Status) {
		t.Helper()
		if err := store.Modify(f.ID, func(f *feature.Feature) error { f.Status = status; return nil }); err != nil {
			t.Fatal(err)
		}
	}
	setStatus(feature.StatusCodeReady)
	setStatus(feature.StatusPublished)
	// Restart both the store and observer; no in-memory deduplication survives.
	store = feature.NewStore(dir)
	store.SetMutationObserver(&Observer{enabled: true, localEnabled: true, emitter: NewEmitter(dir)})
	setStatus(feature.StatusImplementing)
	setStatus(feature.StatusCodeReady)
	setStatus(feature.StatusDone)
	secondStart := time.Now().Add(-10 * time.Second)
	_, err := store.SealAndForkRun(f.ID, func(r *feature.Run) error {
		now := time.Now()
		r.SealedAt = &now
		r.SealReason = feature.SealReasonRewind
		return nil
	}, func(r *feature.Run) (*feature.Run, error) {
		return &feature.Run{RunNumber: 2, StartedAt: &secondStart, CarriedFromRun: 1, Committing: true}, nil
	}, func(_, _ *feature.Run) error { return nil })
	if err != nil {
		t.Fatal(err)
	}
	fresh, err := store.Load(f.ID)
	if err != nil {
		t.Fatal(err)
	}
	if fresh.Run().OutputReadyAt != nil || fresh.Run().DeliveredAt != nil {
		t.Fatal("fork inherited milestone timestamps")
	}
	setStatus(feature.StatusImplementing)
	setStatus(feature.StatusCodeReady)
	setStatus(feature.StatusPublished)
	data, err := os.ReadFile(filepath.Join(dir, f.ID, "events.jsonl"))
	if err != nil {
		t.Fatal(err)
	}
	milestones := map[int]map[string]int{1: {}, 2: {}}
	for _, line := range strings.Split(strings.TrimSpace(string(data)), "\n") {
		var event Event
		if err := json.Unmarshal([]byte(line), &event); err != nil {
			t.Fatal(err)
		}
		if event.EventType == "feature.run_sealed" && event.Data["run_number"] == float64(1) && event.RunNumber != 1 {
			t.Fatalf("sealed run attributed to new run: %+v", event)
		}
		if event.EventType != "feature.output_ready" && event.EventType != "feature.delivered" {
			continue
		}
		milestones[event.RunNumber][event.EventType]++
		limit := int64(65000)
		if event.RunNumber == 2 {
			limit = 15000
		}
		if event.DurationMs < 0 || event.DurationMs > limit {
			t.Fatalf("milestone measured from feature creation instead of run start: %+v", event)
		}
	}
	for run, counts := range milestones {
		if counts["feature.output_ready"] != 1 || counts["feature.delivered"] != 1 {
			t.Errorf("run %d milestones=%v", run, counts)
		}
	}
}

func TestMetricErrorsUseCanonicalCatalog(t *testing.T) {
	for _, code := range errcat.Codes() {
		if got := metricEnum("error_code", string(code)); got != string(code) {
			t.Errorf("known code %s normalized to %s", code, got)
		}
	}
	if got := metricEnum("error_code", "private failure text"); got != "other" {
		t.Fatalf("unbounded error dimension: %s", got)
	}
	if got := metricEnum("error_class", "needs_action"); got != "needs_action" {
