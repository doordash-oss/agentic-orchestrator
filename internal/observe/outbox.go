...
func (o *eventOutbox) run(ctx context.Context) {
	defer close(o.done)
	backoff := time.Second
	for {
...
func (o *eventOutbox) readBatch() ([]wideRecord, outboxCursor, outboxCursor, error) {
	...
	for si := start; si < len(segments) && len(records) < outboxBatchRecords; si++ {
		...
		if batchFull || len(records) >= outboxBatchRecords || total >= outboxBatchBytes {
			break
		}
	}
...
func sanitizeWideEvent(evt Event) wideRecord {
	...
	for _, field := range []*string{&evt.FeatureID, &evt.SessionID, &evt.RepoName, &evt.SpanID, &evt.ParentSpanID, &evt.EventType, &evt.Phase, &evt.Status} {
		*field, _, _ = sanitizeWideString(*field)
	}
