  ...
  const script = [
    ...
    ...(rebaseProvider
      ? [
          ...
          '  _feedback=$(printf "%s\\n" "$_context" | sed -n "s#.*\\(/[^\\\"\\\\ ]*/review-feedback\\.md\\).*#\\1#p" | head -n 1)',
          '  if [ -z "$_feedback" ]; then',
          ...
          '  _plan=$(printf "%s\\n" "$_agentico_prompt" | sed -n "s#.*\\(/[^\\\"\\\\ ]*/rebase-plan\\.md\\).*#\\1#p" | head -n 1)',
          ...
          '      if git -C "$_dir" status --porcelain | grep -q "^UU README.md"; then',
          '        printf \'# local-core\\nconflicting change on main\\nmerged change from feature\\n\' > "$_dir/README.md"',
          '        git -C "$_dir" -c user.name="Agentico E2E" -c user.email=e2e@example.invalid add README.md',
          '        GIT_EDITOR=true git -C "$_dir" -c user.name="Agentico E2E" -c user.email=e2e@example.invalid rebase --continue',
          '      fi',
          '    fi',
          '    _prev="$_dir"',
          '  done',
          ...
          '    exit 0',
          '    ;;',
          '  *"review-feedback.md"*|*"helper_dir"*|*"iteration_dir"*)',
