...
func TestFinalReviewPromptBranches(t *testing.T) {
	tests := []struct {
		...
		{
			name: "final_review_phase_variant_includes_testing_contract",
			input: FinalReviewUserInput{
				Iteration:           1,
				IsCycleReview:       false,
				DiffBase:            "main",
				RoadmapPath:         "/roadmap.md",
				ExitCriteria:        "Relevant tests pass.",
				TestingContractPath: "/testing-contract.yaml",
				VerificationPath:    "/verification-report.yaml",
		...
		},
		{
			...
			input: FinalReviewUserInput{
				...
				VerificationPath: "/cycle/verification.yaml",
				FeedbackPath:     "/cycle/review-feedback.md",
				Publishable:      false,
				PreviousFeedback: "Use a darker hover state.",
			},
			wantContains: []string{"## Current Cycle Focus", "Tweak: button color only.", "**Verification report**: /cycle/verification.yaml", "NOTE: Local-only repository"},
			wantOmit:     []string{"**Testing contract**"},
		},
	}
