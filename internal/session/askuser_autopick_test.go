...
func TestDecideAskUserAutoPick(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "prose confidence in description substitutes for the missing field",
			purpose:     ports.AskUserAutoPickPurposePhasePlanCreator,
			inquireness: feature.InquirenessNone,
			input:       askInput(`{"question":"Pre-fill?","options":[{"label":"Pane adds a row (Recommended)","description":"Explicit and consistent with the rule. Confidence 0.70."},{"label":"Server appends","description":"Hidden behaviour. Confidence 0.35."}]}`),
			wantPick:    true,
			wantAnswer:  "Pane adds a row (Recommended)",
			wantConf:    0.7,
		},
		{
			name:        "prose confidence suffix on label is stripped from the answer",
			purpose:     ports.AskUserAutoPickPurposeInquire,
			inquireness: feature.InquirenessNone,
			input:       askInput(`{"question":"Scope?","options":[{"label":"Repo [confidence: 0.8] (Recommended)","description":"x"},{"label":"All [confidence: 0.1]","description":"y"}]}`),
			wantPick:    true,
			wantAnswer:  "Repo (Recommended)",
			wantConf:    0.8,
		},
		{
			name:        "structured field wins over prose",
			purpose:     ports.AskUserAutoPickPurposeInquire,
			inquireness: feature.InquirenessNone,
			input:       askInput(`{"question":"Scope?","options":[{"label":"Repo","description":"Confidence 0.9","confidence":0.3},{"label":"All","description":"Confidence 0.1","confidence":0.6}]}`),
			wantPick:    true,
			wantAnswer:  "All",
			wantConf:    0.6,
		},
		{
			name:        "option without any confidence declines",
			purpose:     ports.AskUserAutoPickPurposeInquire,
			inquireness: feature.InquirenessNone,
			input:       askInput(`{"question":"Scope?","options":[{"label":"Repo","description":"Confidence 0.9"},{"label":"All","description":"no score here"}]}`),
		},
		{
