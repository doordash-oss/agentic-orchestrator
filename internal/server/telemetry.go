...
func withHTTPMetrics(next http.Handler, metrics HTTPMetrics) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.ObserveHTTPRequest(route, telemetryMethod(r.Method), status, time.Since(started), r.ContentLength, mw.bytes)
	})
}
			...
			if len(parts) > 6 {
				...
				case "runs":
					...
					if len(parts) > 8 && parts[7] == "artifacts" {
						...
						if parts[7] != "artifacts" && parts[7] != "logs" && parts[7] != "sessions" {
							parts[7] = "{other}"
						}
						parts = parts[:8]
					}
...
func knownStaticTelemetryRoute(path string) bool {
	for _, route := range topLevelServerRoutes {
		if !strings.HasSuffix(route.pattern, "/") && path == route.pattern {
			return true
		}
	}
	return false
