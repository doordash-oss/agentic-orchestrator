...
// TestRoadmapPlanningLoopSingleDeliveryTwoRowTableRoutesToReviser proves an
// attempt whose roadmap carries a structurally valid two-row `## Pull
// Requests` table on a single-delivery feature is recorded as changes
// requested with feedback naming the section, and a later attempt writing a
// one-row table proceeds to validator approval.
func TestRoadmapPlanningLoopSingleDeliveryTwoRowTableRoutesToReviser(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
	...
	// The first attempt writes a roadmap whose `## Pull Requests` table is
	// structurally valid but splits a single-delivery feature across two
	// rows. Like the Phase-1 invalid-table test, the script answers every
	// commit-violation nudge with a fresh success outcome so the attempt can
	// only end deterministically. Once the loop forks attempt-02 the script
	// writes a one-row roadmap and exits normally.
	roadmapTwoRowText := "# Roadmap\n\n## Phase 1: Skeleton\n\n### Goal\nShip the skeleton.\n\n## Phase 2: Polish\n\n### Goal\nPolish it.\n\n## Pull Requests\n\n" +
		"| # | Title | Phases | Rationale |\n|---|---|---|---|\n" +
		"| 1 | Skeleton | 1 | First slice. |\n| 2 | Polish | 2 | Second slice. |\n"
	roadmapOneRowText := "# Roadmap\n\n## Phase 1: Skeleton\n\n### Goal\nShip the skeleton.\n\n## Phase 2: Polish\n\n### Goal\nPolish it.\n\n## Pull Requests\n\n" +
		"| # | Title | Phases | Rationale |\n|---|---|---|---|\n" +
		"| 1 | Whole feature | 1-2 | Delivered as one pull request. |\n"
	writeOneRowRoadmap := fmt.Sprintf(`cat > %q <<'ROADMAPEOF'
%s
ROADMAPEOF
`, filepath.Join(planDir, "roadmap.md"), roadmapOneRowText)
	assistantOutcomeEcho := strings.Split(testutil.JSONLSuccess, "\n")[0]
	planScript := testutil.WriteScript(t, scriptsDir, "plan.sh", fmt.Sprintf(`%s
if [ -d %q ]; then
%s
...
`, testutil.JSONLInit, filepath.Join(planDir, "attempt-02"),
		writeOneRowRoadmap, testutil.JSONLSuccess,
		filepath.Join(planDir, "roadmap.md"), roadmapTwoRowText,
		assistantOutcomeEcho, finishOrViolateNudgeCasePattern, assistantOutcomeEcho))
	criticScript := testutil.WriteScript(t, scriptsDir, "critic.sh",
		testutil.JSONLInit+"\n"+testutil.WriteAnyValidatorApproved(tmpDir)+"\n"+testutil.JSONLSuccess+"\n")
	...
	if !strings.Contains(string(feedback), "single pull request") {
		t.Errorf("attempt-01 feedback = %q, want feedback naming the single-pull-request delivery", feedback)
	}
}
