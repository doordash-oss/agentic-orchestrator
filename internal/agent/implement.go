...
func runReviewGate(cfg ImplementConfig, sm ports.SessionManager, iteration int, iterDir string, parsed *ParsedProgress, reviewCtx observe.SpanContext) (ReviewStatus, string, error) {
	...
	helperResult, err := helper.RunReadOnlyReviewHelper(context.Background(), ReviewHelperConfig{
		...
		HandoffPath:             filepath.Join(reviewDir, ReviewProgressHandoffFilename),
		MaxConsecNoProgress:     cfg.MaxConsecNoProgress,
		MaxConsecHandoffFails:   cfg.MaxConsecFails,
		ContextHandoffIteration: iteration,
		ContinuationPaths:       []string{cfg.PlanPath, progressPath, verificationReportPath, contractPath},
	})
	if err != nil {
		feedback := ""
		if helperResult != nil {
