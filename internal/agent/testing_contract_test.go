...
func TestCompileTestingContractTimeoutTokenOverridesDefault(t *testing.T) {
	plan := strings.Join([]string{
		"#### Automated Verification:",
		"- [ ] [timeout: 30m] Slow suite passes: `make e2e`",
		"- [ ] Build all packages: `go build ./...`",
	}, "\n")
	contract := CompileTestingContract(plan, "plan.md", "tdd-fill-in")
	if len(contract.Items) != 2 {
		t.Fatalf("CompileTestingContract() got %d items, want 2", len(contract.Items))
	}
	if contract.Items[0].Run == nil || contract.Items[0].Run.Timeout != "30m" {
		t.Fatalf("first item run = %+v, want sanctioned 30m timeout", contract.Items[0].Run)
	}
	if contract.Items[1].Run == nil || contract.Items[1].Run.Timeout != testingContractDefaultRunTimeout {
		t.Fatalf("second item run = %+v, want default %s timeout", contract.Items[1].Run, testingContractDefaultRunTimeout)
	}
}

func TestCompileTestingContractMultiRepoTimeoutToken(t *testing.T) {
	contract := CompileTestingContractMultiRepo(MultiRepoContractInput{
		Repos:    []string{"api"},
		PlanText: "### Automated Verification\n- [ ] [repo: api] [timeout: 45m] API suite: `go test ./...`\n",
		PlanPath: "plan.md",
	})
	if len(contract.Items) != 1 {
		t.Fatalf("CompileTestingContractMultiRepo() got %d items, want 1", len(contract.Items))
	}
	item := contract.Items[0]
	if item.Repo != "api" || item.Run == nil || item.Run.Timeout != "45m" {
		t.Fatalf("item = %+v, want repo-scoped 45m timeout", item)
	}
}

func TestWriteTestingContractIsBanneredReadOnlyAndRoundtrips(t *testing.T) {
	...
	if !strings.HasPrefix(string(data), "# GENERATED by Agentico") {
		t.Fatalf("contract missing harness-owned banner:\n%s", data)
	}
	...
	if info.Mode().Perm() != 0o444 {
		t.Fatalf("contract mode = %v, want read-only 0444", info.Mode().Perm())
	}
	got, err := ReadTestingContract(path)
	if err != nil {
		t.Fatalf("ReadTestingContract() error = %v", err)
	}
	if len(got.Items) != 1 || got.Items[0].Command != "go build ./..." {
		t.Fatalf("roundtripped contract = %+v", got)
	}
	// Regeneration over an existing read-only contract succeeds.
	if err := WriteTestingContract(path, contract); err != nil {
		t.Fatalf("second WriteTestingContract() error = %v", err)
	}
}
