...
| **Large** | KB Build → Inquire → Research → Brainstorm → Plan → Implement → Review → Publish | High | Yes | No |
| **Moonshot** | KB Build → Inquire → Research → Brainstorm → Plan → Implement → Review → Publish | Max | No | No |
...
### Brainstorm
Generates and evaluates implementation approaches. Considers trade-offs, risks, and alternatives before committing to a design direction.
...
```
Created → BuildingKB → Inquiring → Researching → Brainstorming → Planning
...
| `Brainstorming` | Brainstorm phase running |
| `BrainstormReady` | Brainstorm complete, ready for planning |
...
| `ResearchNeedsReview` | Brainstorm phase |
...
| Research Review | Brainstorm | Off | Off | Off |
