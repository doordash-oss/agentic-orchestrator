...
// testingContractBanner marks the file as harness-owned; it is a YAML comment
// so readers and fingerprints handle it transparently.
const testingContractBanner = "# GENERATED by Agentico — harness-owned; regenerated every iteration. Amend the phase plan instead.\n"
...
func WriteTestingContract(path string, contract TestingContract) error {
	...
	data = append([]byte(testingContractBanner), data...)
...
// testingContractDefaultRunTimeout applies when the plan's verification step
// carries no sanctioned [timeout: <duration>] token.
const testingContractDefaultRunTimeout = "10m"

func testingContractRunFor(source, command, timeout string) *TestingContractRun {
	switch strings.TrimSpace(source) {
	case testingContractPlanSource, testingContractCrossRepoSource:
		if strings.TrimSpace(timeout) == "" {
			timeout = testingContractDefaultRunTimeout
		}
		return &TestingContractRun{Shell: strings.TrimSpace(command), Cwd: ".", Timeout: strings.TrimSpace(timeout)}
	default:
		return nil
	}
}
