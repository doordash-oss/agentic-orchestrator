...
func (o *Observer) FeatureMutated(m feature.Mutation) {
	...
	if current.ErrorCode != "" {
		base["error_code"] = current.ErrorCode
		base["error_class"] = current.ErrorClass
	}
	emit := func(kind, status string, extra map[string]any) {
		...
		eventSC := sc
		if run, ok := extra["run_number"].(int); ok {
			eventSC = sc.WithRun(run)
		}
		_ = o.emit(eventSC, Event{Timestamp: m.At, TraceID: sc.TraceID, SpanID: NewSpanID(), EventType: kind,
	...
	if m.Before.Status != m.After.Status {
		...
		case feature.StatusCodeReady:
			if m.Before.OutputReadyAt == nil && m.After.OutputReadyAt != nil {
				emit("feature.output_ready", "ready", map[string]any{"_duration_ms": sinceMillis(m.After.StartedAt, *m.After.OutputReadyAt)})
				o.collectOutputStatsAsync(sc, m.After.OutputRepos, "output_ready")
			}
		case feature.StatusPublished, feature.StatusDone:
			if m.Before.DeliveredAt == nil && m.After.DeliveredAt != nil {
				emit("feature.delivered", "delivered", map[string]any{"_duration_ms": sinceMillis(m.After.StartedAt, *m.After.DeliveredAt)})
		...
		case feature.StatusFailed:
			if m.After.Phase == feature.PhasePublish {
				emit("publish.failed", "failed", map[string]any{"error_code": m.After.ErrorCode, "error_class": m.After.ErrorClass})
			}
		}
	}
