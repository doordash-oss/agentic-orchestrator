...
- **Output ready** is the first transition to `CodeReady` in each run.
- **Delivered** is the first transition to `Published` or `Done` in each run.
- **Agentico failure** is terminal `Failed`, classified by canonical `error_code` and `error_class`.
...
Readiness and delivery timestamps are persisted in the active run and survive
retries and restarts. A rewind starts a new set of milestones. Time-to-milestone
histograms measure elapsed time from that run's `started_at`; an unknown start
omits the duration sample. Existing runs are not retrospectively backfilled.
Terminal-status entries and rewinds are observations, not unique-run counts.
...
phase, status/outcome, catalog error code/class, provider, allowlisted model,
...
parent/child kind, provider/model/effort, outcome and canonical error identity. Numeric
...
must deduplicate by `agentico.event.id`. The queue guarantees replay of
successfully enqueued records until acknowledgement or explicit retention loss;
feature persistence and queue appends are not one transaction, so a process
crash between them can omit a lifecycle observation. Collector acknowledgement
does not guarantee downstream storage; collector rejection/sampling policies
must be configured and monitored separately.
