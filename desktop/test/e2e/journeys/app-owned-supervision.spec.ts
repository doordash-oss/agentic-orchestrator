...
test('app-owned transient recovery, crash-loop exhaustion, and manual retry', async ({}, testInfo) => {
  ...
  try {
    ...
    await evidenceShot(handle, 'phase2-transient-crash-recovered');
    ...
    await evidenceShot(handle, 'phase2-crash-loop-exhausted-shell');
    ...
    await evidenceShot(handle, 'phase2-manual-retry-recovered');
  } finally {
    if (handle !== null) {
