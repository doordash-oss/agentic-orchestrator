...
// comment endpoint, and per-PR GraphQL thread maps and resolutions.
...
// TestReviewFeedbackTailDivergedRepublishBlocksReplies proves a repository
// whose republish fails with a diverged remote records the tail-incomplete
// warning, posts no replies, and keeps its comments out of the addressed
// ledger, while the other repository is replied to and the parent still
// ends Published with the tail settled.
func TestReviewFeedbackTailDivergedRepublishBlocksReplies(t *testing.T) {
	...
	// The parent still ends Published with the settled marker.
	if parent.Status != feature.StatusPublished {
		t.Errorf("parent status = %s, want Published", parent.Status)
	}
	if !child.Parent.Transaction.TailSettled {
		t.Errorf("tail-settled marker = false, want true")
	}
}
